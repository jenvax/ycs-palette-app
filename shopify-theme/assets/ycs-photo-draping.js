...
replaceButtons.forEach(function (btn) {
  ...
  function normalizePaletteCode(value) {
    ...
    if (raw.toUpperCase().indexOf(CUSTOM_PALETTE_PREFIX) === 0) {
      return CUSTOM_PALETTE_PREFIX + raw.slice(CUSTOM_PALETTE_PREFIX.length);
    }
    return raw.toUpperCase();
  }

  function isCustomPaletteCode(code) {
    return String(code || '').toUpperCase().indexOf(CUSTOM_PALETTE_PREFIX) === 0;
  }

  function getCustomPaletteIdFromCode(code) {
    return String(code || '').slice(CUSTOM_PALETTE_PREFIX.length);
  }
  ...
  function getCustomPalettesApiUrl(action) {
    const query = new URLSearchParams();
    query.set('action', action || (IS_STYLE_MASTERS && !IS_ADMIN ? 'vipList' : 'list'));
    query.set('customerId', CUSTOMER_ID || VIEWER_CUSTOMER_ID);
    return APP_BASE_URL.replace(/\/$/, '') + '/api/custom-palettes?' + query.toString();
  }

  async function fetchCustomPalettes() {
    if (!APP_BASE_URL || !CUSTOMER_ID || (!IS_STYLE_MASTERS && !IS_ADMIN)) {
      state.customPalettes = [];
      return;
    }

    try {
      const response = await fetch(getCustomPalettesApiUrl(IS_STYLE_MASTERS && !IS_ADMIN ? 'vipList' : 'list'), {
        credentials: 'omit'
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Could not load custom palettes');
  ...
  async function fetchCustomPaletteColors(paletteCode) {
    ...
    const existingPalette = state.customPalettes.find(function (item) {
      return String(item.id) === paletteId;
    });

    const palette = existingPalette || null;
    return ((palette && palette.colors) || []).map(function (join, index) {
  ...
  function getQueryParams() {
    const params = new URLSearchParams(window.location.search);
    return {
      palette: normalizePaletteCode(params.get('palette') || ''),
  ...
  function getPaletteFromUrl() {
    const params = new URLSearchParams(window.location.search);
    return normalizePaletteCode(params.get('palette') || '');
  }
  ...
  ownedPalettes = customPaletteCodes.concat(ownedPalettes).filter(function (code, index, list) {
...
function updateFilterArrows() {
  ...
  async function initializePaletteUi() {
